...
def extract_stats_from_logs(log_file_path):
    ...
    cpg_regex = r"Total number of CpGs: (\d+)"
    ...
    for session in file_sessions:
        ...
        cpg_match = re.search(cpg_regex, session)
        ...
        if file_name_match and paired_reads_match and alignment_rate_match:
            stats.append({
                ...
                "Number of CpGs": int(cpg_match.group(1)) if cpg_match else 0
            })
...
def create_plots(df, output_dir):
    """Create plots of read counts and alignment rates."""
    # Create figure
...
def main():
    parser = argparse.ArgumentParser(description='Analyze methylpy log files and generate summary statistics')
